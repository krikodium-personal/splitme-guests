...
const ConfirmationView: React.FC<ConfirmationViewProps> = ({ onRestart, onBackToStart, guests = [], splitData, tableNumber, activeOrderId, currentGuestId, waiter }) => {
 ...
 return (
 <div className="relative flex min-h-screen w-full flex-col max-w-md mx-auto bg-transparent overflow-hidden pb-32">
 <div className="flex items-center px-4 pt-6 pb-2 justify-between z-10 sticky top-0 bg-background-dark/80 backdrop-blur-md">
 <button onClick={onRestart} className="text-white flex size-10 shrink-0 items-center justify-center rounded-full active:bg-white/10 transition-colors"><span className="material-symbols-outlined">arrow_back</span></button>
 <h2 className="text-white text-lg font-bold flex-1 text-center pr-10">Estado de Cuenta</h2>
 </div>
 ...
 <h2 className="text-white tracking-tight text-[28px] font-bold leading-tight px-4 text-center">¡Cuenta Saldada!</h2>
 </>
 ) : (
 <>
 ...
 <h2 className="text-white tracking-tight text-[28px] font-bold leading-tight px-4 text-center">Pendiente de Pago</h2>
 </>
 )}
 ...
 <div className="flex min-w-[111px] flex-1 basis-[fit-content] flex-col gap-1 rounded-2xl bg-surface-dark border border-border-dark p-4 items-center text-center shadow-sm">
 <p className="text-white tracking-tight text-2xl font-bold leading-tight">${formatPrice(totalAmount)}</p>
 <div className="flex items-center gap-1.5"><span className="material-symbols-outlined text-text-secondary text-sm">receipt_long</span><p className="text-text-secondary text-text-secondary text-sm font-medium">Total Cuenta</p></div>
 </div>
 </div>

 {!allGuestsPaid && (
 <div className="px-4 py-2">
 <div className="w-full rounded-2xl bg-surface-dark border border-border-dark p-4 flex items-center justify-between shadow-md relative overflow-hidden group cursor-pointer">
 ...
 <div className="flex items-center justify-between px-4 pb-2 pt-4"><h3 className="text-white text-lg font-bold leading-tight">Pagos de la división de la cuenta</h3></div>

 <div className="flex flex-col gap-3 px-4 pb-4 overflow-y-auto no-scrollbar">
 {isLoading ? (
 <div className="flex items-center justify-center p-8">
 <p className="text-text-secondary text-sm">Cargando estado de invitados...</p>
 </div>
 ) : (
 ...
 return (
 <div key={guest.id} className={`flex items-center p-3 rounded-2xl bg-surface-dark border border-border-dark shadow-sm transition-opacity ${isPaid ? 'opacity-60' : ''}`}>
 ...
 <div className="ml-3 flex-1"><p className="text-white font-bold">{isMe ? 'Tú' : guest.name.split(' ')[0]}</p><p className={`${isPaid ? 'text-green-400' : isMe ? 'text-primary' : 'text-text-secondary'} text-sm font-medium`}>{isPaid ? `Pagado $${formatPrice(amount)}${paymentMethod ? ` · ${paymentMethod}` : ''}` : `Debe $${formatPrice(amount)}`}</p></div>
 {!isPaid && (
 <>
 <span className="bg-white/5 text-text-secondary text-xs px-2 py-1 rounded-full font-medium">{isMe ? 'Pendiente' : 'En proceso'}</span>
 {!isMe && (
 <button 
 onClick={() => handleSharePayment(guest.name, amount, guest.id)}
 className="size-10 rounded-xl bg-white/5  flex items-center justify-center text-text-secondary text-text-secondary active:scale-90 transition-all hover:bg-primary/20 hover:text-primary border border-white/5 ml-2"
 ...
 <div className="fixed bottom-0 left-0 w-full bg-background-dark border-t border-border-dark p-4 shadow-2xl z-50">
 {!shouldShowPayCta ? (
 <button 
 ...
 className="w-full bg-primary hover:bg-primary-dark text-black font-bold text-lg h-14 rounded-xl flex items-center justify-center gap-2 shadow-lg shadow-primary/20 group"
 >
 <span>Ir al inicio</span>
 <span className="material-symbols-outlined group-hover:translate-x-1 transition-transform">arrow_forward</span>
 </button>
 ) : (
 <button onClick={onRestart} className="w-full bg-primary hover:bg-primary-dark text-black font-bold text-lg h-14 rounded-xl flex items-center justify-between px-6 shadow-lg shadow-primary/20 group">
