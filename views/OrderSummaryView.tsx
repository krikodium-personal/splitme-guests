...
const OrderSummaryView: React.FC<OrderSummaryViewProps> = ({ 
  guests, cart, batches, onBack, onNavigateToCategory, onEditItem, onSend, onPay, isSending = false, onUpdateQuantity, menuItems, tableNumber
}) => {
  ...
  const confirmedByBatch = useMemo(() => {
    ...
    })).filter(b => b.items.length > 0).reverse(); // Recientes arriba
  ...
  return (
    <div className="bg-background-dark text-white font-display h-screen flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-48 no-scrollbar">
                  ...
                  <div key={guest.id} className="bg-surface-dark rounded-3xl p-4 border border-primary/20 shadow-lg shadow-primary/5">
                    ...
                    <div className="space-y-3">
                      {guestPending.map(item => {
                        ...
                        const hasCustomizations = (item.extras?.length || 0) > 0 || (item.removedIngredients?.length || 0) > 0;
        ...
        {/* SECCIÓN: HISTORIAL POR LOTES */}
        {confirmedByBatch.length > 0 && (
          <div className="space-y-8 animate-fade-in">
            <h3 className="text-text-secondary text-[10px] font-black uppercase tracking-[0.3em] pl-2">Historial de Pedidos</h3>
                      ...
                      {batch.items.map(item => {
                        ...
                        return (
                          <div key={item.id} className="p-4 flex flex-col gap-2">
                            <div className="flex items-center gap-4">
                              <div className="size-11 rounded-xl bg-cover bg-center shrink-0 grayscale-[0.5] opacity-60 border border-white/5" style={{ backgroundImage: `url("${dish?.image_url}")` }}></div>
                              <div className="flex-1 min-w-0">
                            ...
                            {hasCustomizations && (
                              <div className="flex flex-wrap items-center gap-1.5 pl-15 opacity-40">
                                {item.extras?.map(ex => (
      ...
      <div className="fixed bottom-0 left-0 w-full p-4 bg-background-dark border-t border-white/5 z-20 space-y-3 shadow-[0_-10px_30px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex flex-col gap-3">
          {pendingItems.length > 0 && (
            <button 
              onClick={onSend} 
              disabled={isSending}
              className="w-full h-16 bg-primary text-background-dark rounded-2xl flex items-center justify-between px-8 shadow-xl shadow-primary/20 font-black active:scale-[0.98] transition-all"
            >
              <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className={`w-full h-14 rounded-2xl font-black flex items-center justify-center gap-2 transition-all ${
                pendingItems.length > 0 
                  ...
                  : 'bg-primary text-background-dark shadow-xl shadow-primary/20 active:scale-[0.98]'
              }`}
            >
          ...
          {cart.length === 0 && (
            <button onClick={onBack} className="w-full h-16 bg-white/5 text-white border border-white/10 rounded-2xl font-black flex items-center justify-center gap-2">
